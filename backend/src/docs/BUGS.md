...
## 5. Video uploads auto‑approved (no moderation for normal users)

**Where:** `src/modules/videos/video.service.js` – `createVideo()`

**Problem:** Every new video is created with `status = "approved"`. Requirement is that normal users’ uploads are `pending` until admin approves.

**Fix:** Set `status = (user.role === 'admin') ? 'approved' : 'pending'`. Ensure list/detail only show `status === 'approved'` to the public (already done). Admin approve/reject/hide flows already exist.
...
| Priority | Bug | Impact |
|----------|-----|--------|
| P0 | #1 Auth middleware JWT field | All protected routes broken after login |
| P0 | #3 JWT_SECRET in env | Runtime auth failures if env missing |
| P1 | #2 Malformed Authorization | Cleaner errors, avoid edge-case crashes |
| P1 | #4 Admin self-role change | Security |
| P1 | #5 Video pending for non-admin | Moderation policy |
| P2 | #6 Livestream mute | Moderation |
| P2 | #7 Livestream kick | Moderation |
| P3 | #8 OTP (only if product requires it) | Feature completeness |
