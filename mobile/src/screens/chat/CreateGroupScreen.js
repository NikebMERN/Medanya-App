  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      if (taken) {
        Alert.alert(
          "Name not allowed",
          "This name is already used by a user. Please choose a different group name."
        );
        return;
      }
    } catch {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        err.response?.data?.error?.message || err.message || "Failed to create group."
      );
    } finally {
      setSubmitting(false);
    }
  }, [groupName, selectedIds, navigation]);
